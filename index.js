...
exports = module.exports = async function sited_test(sitedPath, key, callback) {
    ...
    async function home_test(cback) {
        ...
        await source.getNodeViewModel(
            ...
            isUpdate,
            async (code) => {
                // code == 1 表示请求url有返回html但不代表节点解析出正确数据; code == -1 表示请求url没有响应; code == -2 表示请求url过程出错且没有缓存;                console.log(String(code) + 'codecode');
                if (code == 1) {
                    await cback(doTest);
                } else {
    ...
    async function search_test(cb) {
        ...
        var doTest = async () => {
            ...
            console.log(JSON.stringify(viewModel.list.slice(0, 2)) + ' ......');
    ...
    async function book_test(bookUrl, from_where, cb) {
        ...
        var doTest = async () => {
            ...
            if (dtype < 4) {
                console.log(
                    'name:' +
                        (viewModel.name ? String(viewModel.name) + ' ,' : '') +
                        '\nauthor:' +
                        (viewModel.author
                        ...
                        '\nintro:' +
                        (viewModel.intro
                            ? String(viewModel.intro).slice(0, 20) + ' ...... ,'
                            : '') +
                        '\nlogo:' +
                        (viewModel.logo ? String(viewModel.logo) + ' ,' : '') +
                        '\nupdateTime:' +
                        (viewModel.updateTime
                            ? String(viewModel.updateTime) + ' ,'
                            : '') +
                        '\nisSectionsAsc:' +
                ...
                if (viewModel.sectionCount() == 0) {
                    console.log('此内容已下架@_@?a R.string.error_no_content');
                }
                await cb();
                return;
            } else {
        ...
        var SdSourceCallback = async (code) => {
        ...
        if (dtype <= 7) {
            ...
            if (config.hasItems()) {
                for (let item of config._items) {
                    console.log('book[8].item ' + item.attrs._items.toJson());
