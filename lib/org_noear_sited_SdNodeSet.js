        ...
        for (let i = 0, len = xList.length; i < len; i++) {
            ...
            if (n1.nodeType == 1) {
                ...
                if (Object.keys(e1.attribs).length) {
                    // 说明element的子节点e1是Node类型例如<hots xxx />、<tags title=yyy><item /></tags>
                    ...
                    // java版e1.getChildNodes().getLength()>1，说明element的子节点e1是NodeSet类型如<home>，标签之间要有文本节点(即换行、空白)不然<book><book xxx /></book>不会大于1就不能识别为NodeSet。tags不会是NodeSet因为含有title属性占位。
