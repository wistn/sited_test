...
class SdNode {
    constructor(source) {
        ...
        this.name = null; // 节点名称
        this.key = null; // 自定义关键字
        this.title = null; // 标题
        this.txt = null; // txt//一用于item
        this.logo = null; // logo
        this.expr = null;
        this.group = null;
        this.lib = null;
        ...
        this.referer = null;
        this.cookie = null;
        this.header = null; // http header 头需求: cookies|accept
        this.method = null; // http method
        this._encode = null; // http 编码
        this._ua = null; // http ua
        this.cache = 1; // 单位为秒(0不缓存；1不限时间)
        // parse
        this.onParse = null; // 解析函数
        this.onParseUrl = null; // 解析出真正在请求的Url
        // build
        ...
        this.addCookie = null; // 需要添加的cookie
        this.addKey = null; // 需要添加的关键字
        this.addPage = 0; // 需要添加的页数值
        // ext prop (for post)
        this._isEmpty = false;
        this._items = null;
        this._adds = null;
    ...
    buildForAdd(cfg, p) {
        ...
        this.onParseUrl = this.attrs.getString2('onParseUrl', 'parseUrl');
        {
            let temp = this.attrs.getString('cache'); // js版增加适配多search节点搜索的cache
            if (TextUtils.isEmpty(temp) == false) {
                let len = temp.length;
                if (len == 1) {
                    this.cache = Number(temp);
